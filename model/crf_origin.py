...
class CRF(nn.Module):
    ...
    def neg_log_likelihood_loss(self, feats, mask, tags):
        ...
        #length_mask = None
        #log_potentials = self.transitions.expand(batch_size,maxlen+1,-1,-1)
        ...
        ind = tuple(i for i in range(self.tagset_size+2) if i!=self.tagset_size+2+STOP_TAG )
        #log_potentials[torch.arange(batch_size),length_mask-2,:,:][:,:,ind]-=1000
        for i in range(batch_size):
            log_potentials[i,length_mask[i]-2,:,ind]-=1000
        #log_potentials[:,0:-1,:,:]+=feats.view(batch_size,maxlen,1,self.tagset_size+2)
        print(ind)
        ...
        #print(log_potentials.cuda().unsqueeze(-1)[z.unsqueeze(-1).type(torch.bool)])
        #print("dist.log_prob",dist.log_prob(z))
        #print("dist.partition",dist.partition)
        forward_score, scores = self._calculate_PZ(feats, mask)
        gold_score = self._score_sentence(scores, mask, tags)
        #select = log_potentials.cuda().unsqueeze(-1)[z.unsqueeze(-1).type(torch.bool)]
        #print("select sum",select.sum())
        #print("torch.mul sum",torch.mul(log_potentials.cuda(), z.cuda()).sum())
        #print("struct score",dist.score(z))
        #print("gold_score",gold_score)
        #print("sum dist.log_prob(z)",dist.log_prob(z).sum().item())
        #print("forward_score - gold_score",forward_score - gold_score)
        assert(abs(gold_score-(dist.log_prob(z)+dist.partition).sum().item())<0.01)
        ref = forward_score-gold_score
        assert(abs(ref+dist.log_prob(z).sum().item())<0.01)
        # print "batch, f:", forward_score.data[0], " g:", gold_score.data[0], " dis:", forward_score.data[0] - gold_score.data[0]
        # exit(0)
        print(dist.argmax.shape)
        decoded_tag = torch.zeros(batch_size,maxlen)
        argmax = dist.argmax
        for i in range(batch_size):
            tmp = argmax[i].nonzero()[:-1,1]
            assert(tmp.size(0)==length_mask[i]-2)
            decoded_tag[i,0:length_mask[i]-2]=tmp

        return forward_score - gold_score, decoded_tag.cuda()
